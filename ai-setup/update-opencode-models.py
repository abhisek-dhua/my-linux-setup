...
def write_config(path, config, original_raw):
    """Write config preserving JSONC style."""
...
def update_nvidia(config, nvidia_models):
...
def main():
    ...
    config, raw = read_config(config_path)
    ...
    nv_ok = update_nvidia(config, nv_models) if nv_models else False

    if or_ok or nv_ok:
        write_config(config_path, config, raw)
